...
var modalaacc = document.getElementById("myNavbar");
var modalaabb = document.getElementById("blankspace1");
let bottombara = getCookie("bottomnavbar");
if (bottombara == "1") {
 modalaacc.style.display = "block";
 modalaabb.style.display = "block";
}
else if (bottombara == "0") {
modalaacc.style.display = "none";
 modalaabb.style.display = "none";
}
else {
	...
	if (ddeeff >= "310") {
		...
		if (ddeeffa <= "600") {
			if (ddeeffa >= "200") {
			setCookie("bottomnavbar", "2", 30);
 			modalaacc.style.display = "block";
		 	modalaabb.style.display = "block";
			}
			else {
			setCookie("bottomnavbar", "3", 30);
			modalaacc.style.display = "none";
 			modalaabb.style.display = "none";
			}
		}
...
function setCookie(cname,cvalue,exdays) {  const d = new Date();  d.setTime(d.getTime() + (exdays*60*60*60*50*100));  let expires = "expires=" + d.toUTCString();  document.cookie = cname + "=" + cvalue + ";" + expires + ";path=/"; }
function getCookie(cname) {  let name = cname + "=";  let decodedCookie = decodeURIComponent(document.cookie);  let ca = decodedCookie.split(';');  for(let i = 0; i < ca.length; i++) {    let c = ca[i];    while (c.charAt(0) == ' ') {      c = c.substring(1);    }    if (c.indexOf(name) == 0) {      return c.substring(name.length, c.length);    }  }  return ""; }
// THIS FUNCTION IS VERY IMPORTANT

setCookie("location.countrycode", userCountry, 30);
setCookie("location.city", userCity, 30);
setCookie("location.region", userRegion, 30);
setCookie("location.timezone", userTimeZone, 30);
